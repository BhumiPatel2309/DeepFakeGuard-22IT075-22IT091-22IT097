...
def video_pred(threshold=0.5, model='EfficientNetAutoAttB4', dataset='DFDC', frames=100, video_path=None):
    """
    ...
    # loading the weights
    model_url = weights.weight_url['{:s}_{:s}'.format(model,train_db)]
    net = getattr(fornet,model)().eval().to(device)
